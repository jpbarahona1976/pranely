...
from app.core.tokens import create_access_token, decode_token, TokenPayload
...
async def register(
    ...
    """Register a new user with their organization (owner role)."""
...
async def login(
    ...
    # Get user's primary organization (first membership by created_at)
...
@router.post(
    ...
    description="Exchange a valid access token for a new one with extended expiry.",
)
async def refresh(
    ...
    Validates the provided token and issues a new one with extended expiry.
    ...
    # Verify membership if org_id present
    if org_id:
        result = await db.execute(
        ...
        membership = result.scalar_one_or_none()
        if membership:
            role = membership.role.value if membership.role else role
